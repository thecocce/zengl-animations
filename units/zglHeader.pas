...
type
  ...
  zglTPoints2D = array[ 0..$FFFFFF ] of zglTPoint2D;
...
type
  zglTMultiTextureType = record
    Texture: zglPTexture;
    Coords: zglTTextureCoord;
    Rect: zglTRect;

    procedure Update(Scale: Single = 1);
  end;

  zglPMultiTextureArray = ^zglTMultiTextureArray;
  zglTMultiTextureArray = array of zglTMultiTextureType;
...
const
  ...
  TEX_DEFAULT_NEAREST   = TEX_CLAMP or TEX_FILTER_NEAREST or TEX_CONVERT_TO_POT or TEX_CALCULATE_ALPHA;
...
{ zglTMultiTextureType }

procedure zglTMultiTextureType.Update(Scale: Single);
begin
  Coords[0].X := Rect.X * Texture.U;
  Coords[0].Y := Rect.H * Texture.V * Scale;

  Coords[1].X := Rect.W * Texture.U * Scale;
  Coords[1].Y := Rect.H * Texture.V * Scale;

  Coords[2].X := Rect.W * Texture.U * Scale;
  Coords[2].Y := Rect.Y * Texture.V;

  Coords[3].X := Rect.X * Texture.U;
  Coords[3].Y := Rect.Y * Texture.U;
end;
